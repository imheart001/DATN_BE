...
class QuerryController extends Controller
{
    ...
    public function movie_rooms($id_cinema, $date, $filmId)
    {
        ...
        foreach ($movieRooms as $room) {
            $carbonDate = \Carbon\Carbon::parse($room->date);
            $dayNames = [
                'Sunday' => 'Chủ Nhật',
                'Monday' => 'Thứ Hai',
                'Tuesday' => 'Thứ Ba',
            ...
            $engDay = $carbonDate->format('l');
            $viDay = $dayNames[$engDay] ?? $engDay;
            $room->formatted_date = $carbonDate->format('d/m') . ' - ' . $viDay;
        }
    ...
    public function purchase_history_ad_cinema($id_cinema)
    {
        $book_ticket_detail = DB::table('book_tickets as bt')
            ->join('time_details as td', 'td.id', '=', 'bt.id_time_detail')
            ->join('movie_chairs as mc', 'mc.id', '=', 'bt.id_chair')
